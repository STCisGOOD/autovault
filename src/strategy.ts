...
export class StrategyEngine {
  ...
  private filterByRiskTolerance(yields: YieldOpportunity[]): YieldOpportunity[] {
    switch (this.riskTolerance) {
      case 'conservative':
        return yields.filter(y => y.riskRating === 'low');
      case 'moderate':
        return yields.filter(y => y.riskRating !== 'high');
      case 'aggressive':
        return yields; // Accept all risk levels
    }
  }
  ...
  private calculateOptimalDistribution(
    ...
    // Take top opportunities based on risk tolerance
    const maxPositions = this.riskTolerance === 'conservative' ? 3 :
                         this.riskTolerance === 'moderate' ? 5 : 7;

    const topYields = scored.slice(0, maxPositions);
    const totalScore = topYields.reduce((sum, y) => sum + y.score, 0);

    // Allocate proportionally to score, with caps
    for (const yield_ of topYields) {
      const rawAllocation = (yield_.score / totalScore) * totalValue;
      // Cap single position at 40% for conservative, 50% moderate, 60% aggressive
      const maxAllocation = totalValue * (
        this.riskTolerance === 'conservative' ? 0.4 :
        this.riskTolerance === 'moderate' ? 0.5 : 0.6
      );
  ...
  private calculateScore(yield_: YieldOpportunity): number {
    ...
    // TVL adjustment (prefer higher TVL for safety)
    if (yield_.tvl > 50000000) score *= 1.1;
    else if (yield_.tvl < 1000000) score *= 0.8;
