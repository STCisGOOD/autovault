 ...
 * - Proper rate limit headers for client backoff
 */
...
import { getAllowedOrigins, isAllowedOrigin, getDefaultOrigin, getRateLimitConfig } from './config';
import { rateLimiter, getClientIp, getRateLimitHeaders } from './rate-limiter';
...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse) {
  ...
  // CORS - environment-based allowed origins (no localhost in production)
  ...
  if (origin && isAllowedOrigin(origin)) {
    ...
    res.setHeader('Access-Control-Allow-Origin', getDefaultOrigin());
  }
  // If origin not in whitelist, don't set CORS header (browser will block)
  ...
  const rateLimitResult = rateLimiter.check(clientIp, path);
  ...
  if (!rateLimitResult.allowed) {
    ...
    res.end(JSON.stringify({
      ...
      message: `Rate limit exceeded. Try again in ${Math.ceil((rateLimitResult.retryAfterMs || 0) / 1000)} seconds.`,
