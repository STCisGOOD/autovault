...
/**
 * Validate that an object conforms to the Memory schema
 */
function isValidMemory(obj: unknown): obj is Memory {
  ...
  if (typeof mem.id !== 'string') return false;
  if (typeof mem.timestamp !== 'string') return false;
  if (!['reasoning', 'decision', 'learning', 'relationship'].includes(mem.type as string)) return false;
  if (!Array.isArray(mem.tags)) return false;
  if (typeof mem.importance !== 'number' || mem.importance < 0 || mem.importance > 1) return false;

  // ID must match expected format (prevent injection)
  if (!/^mem-\d+-[a-f0-9]+$/.test(mem.id)) return false;

  // Timestamp must be valid ISO date
  if (isNaN(Date.parse(mem.timestamp))) return false;
 ...
 * SECURITY: Validates each memory object before importing
 */
export async function importMemories(data: string): Promise<number> {
  try {
    ...
    for (const mem of parsed.memories) {
      if (isValidMemory(mem)) {
        validMemories.push(mem);
      } else {
        console.warn('Skipping invalid memory object during import');
      }
    }
    ...
    const memories = await loadMemories();
    memories.push(...validMemories);
    await saveMemories(memories);
    return validMemories.length;
  } catch (error) {
    console.error('Memory import failed:', error instanceof Error ? error.message : 'Unknown error');
    return 0;
  }
}
